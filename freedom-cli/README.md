...
两条当前已知边界（如实列出，勿当作"已修"）：① 主密钥的装配码在壳里、生成器在公开源里，故静态逆向出某个壳的人可以拼回那把主密钥——但 1.14.0 起装配码**每个产物一份**（分片与变换随机），针对 A 产物得到的脱壳流程不能直接复用到 B 产物，抬的是自动化与批量脱壳的成本，不是"逆向就是拿不到"；② 前端页面与配置的明文必须交给 webview，能读进程内存者始终看得到那一份（后端源码已在物化后擦除，见上）。
