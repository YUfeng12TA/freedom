...
function checkPlatformProduct({ projectDir, targetDir, plat, appName, hasBackend }) {
  ...
  if (hasBin) {
    ...
    if (head === APP_BIN_MAGIC) {
      fail('app.bin 容器', `${APP_BIN_MAGIC} 属旧代际（密钥在公开源里，清单可被任意伪造）：请用 freedom-cli 1.14.0-preview 及以上重新 build（预览代走 preview 通道：npm i -D @yufengtadian/freedom-cli@preview）`);
