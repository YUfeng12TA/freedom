...
- `security.go` / `anti_debug_*.go` / `securetemp*.go` / `shutdown*.go` — FRDM3 加密容器（容器内嵌构建期随机 salt，PBKDF2-HMAC-SHA256 按「每产物主密钥 + 应用标识 + 盐」60 万次派生 KEK，HMAC 域分离出独立认证钥，AES-256-CTR + Encrypt-then-MAC 覆盖容器头部；`resources/.integrity` 是**发布方 ed25519 私钥签名的 v3 清单**，claims 绑 app.bin 摘要 / 应用标识 / 容器盐 / exe 自身摘要，验签顺序锁死为「锚比对 → 验签 → claims 复查 → exe 自摘要 → 解密」）与调试器检测（Windows 七道信号含 `DR0–DR3` 硬件断点、Linux 一次性 `PTRACE_TRACEME` + `PR_SET_DUMPABLE=0`、macOS 未实现，探测失败一律记未命中）；后端源码也进容器，运行期解密到私有临时目录（`securetemp*.go` 负责按目录名内嵌 PID 回收崩溃/强杀残留），退出时 defer 删除，`shutdown*.go` 接管 SIGINT/SIGTERM 与 Windows `CTRL_CLOSE_EVENT`（先清扫再 exit 130；注意 `-H windowsgui` 产物无控制台，该通道只对控制台子系统构建生效，GUI 产品靠 defer + 下次启动 GC）；**high 模式仅 Tier B（应用自编译壳）可用**：信任锚公钥经 `-X freedom-cli-shell/pkg/freedom.securityAnchorPubHex` 注入，**每产物主密钥不走 `-X`**——CLI 每次 high 构建用 `lib/security.js` 的 `keySlotForBuild` 现场生成一份只属于本产物的装配码 `keyslot_<tag>.go`（3~7 片、切法/变换/参数随机），经 `go build -overlay`（`lib/shell.js` 的 `stageOverlayFiles`）虚拟进 `pkg/freedom` 后编译，临时目录编完即删：**严禁写进 `templates/go`**（那棵树随 npm 包分发、被所有产物共用，落一份就等于把某产物的密钥编进之后所有壳）。壳侧钩子是包级变量 `keySlotAssemble func() []byte`（默认 nil ⇒ 通用预编译壳 Tier A 结构上解不开），`productMaster()` 只认「非 nil、长度恰为 32、非全零」的装配结果，否则 `ok=false` ⇒ 拒跑（70）；旧代 `-X securityMasterCipher` + 公开固定掩码已删除（台账 B-20260925-060：单串 + 公式公开 ⇒ 一份脱壳器通吃所有产物），跨语言锁是 `tests/security-frdm3.test.mjs`「生成的 Go 装配码能编译并跑出同一主密钥」（无 Go 即 skip）与 `security_frdm3_test.go` 的 `TestFRDM3ProductMasterHookContract`（坏值必须拒），契约见 `.liangzu/plans/r6-defense-max/frdm3-contract.md` §7；主密钥取出后由调用方 `defer clearBytes(master)` 立即清零；**派生出的容器密钥（KEK/macKey）不得写进跨调用缓存**（`secureKeyCache` 只服务 FRDM2 黄金向量测试），`decryptAppBinV3` 用 `defer k.clear()` 让它只活在一次解密内——驻留等于给对手"一次内存取样 → 对磁盘产物永久离线解密"的能力（salt/iv 都在容器头，除 KEK 外无第二个秘密输入）；载荷按 `(应用标识, 容器字节哈希)` 记忆化（`securePayloadCache`，验签链每次照跑、缓存只跳过派生+解密，R2 那次"缓存载荷让篡改用例假通过"的教训由 `security_frdm3_test.go` 真机用例②③④与 `security_lifecycle_test.go` 双向锁死），后端源文写入私有临时目录后由 `scrubSecureBackendPayload` 抹掉内存副本（FRDM2 遗留路径另有 `withMasterSecret` 作用域抹零与缓存淘汰前清零）；**安全校验失败一律 `os.Exit(70)`**（`exitSecureFatal`），不得返回 nil 让"拒跑"与"正常退出"同形；**旧代际容器（FRDM1/FRDM2）明确拒绝，禁静默降级**；参数与 freedom-cli `lib/security.js` 跨语言同步，改任一侧必须同步另一侧并重生成共享夹具（`security_frdm3_test.go` 与 `tests/security-frdm3.test.mjs` 读同一份 `tests/fixtures/frdm3-golden.json`；重生成 `FRDM3_REGEN=1 node tests/security-frdm3.test.mjs`，Go 侧出向量 `FRDM3_GO_VECTOR=1 go test -run TestFRDM3EmitGoSignedVector .`）。
